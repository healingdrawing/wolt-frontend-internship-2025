...
// Define the atom to store user coordinates
...
const UserCoordinates: React.FC = () => {
  ...
  // try to get coordinates from Geolocation API
  ...
  const handle_input_change = (e: React.ChangeEvent<HTMLInputElement>, type: 'latitude' | 'longitude') => {
    ...
    if (minus_numbers_period_regex.test(value) || value === '') {
      ...
      if (!wait_list.includes(value)){
        ...
        if (type === 'latitude') {
          set_coordinates((prev) => ({ ...prev, latitude: value === '' ? null: parseFloat(value)}))
        } else {
          set_coordinates((prev) => ({ ...prev, longitude: value === '' ? null: parseFloat(value)}))
        }
      }
    } else {
      if (type === 'latitude') {
  ...
  const handle_input_on_blur = (e: React.FocusEvent<HTMLInputElement>, type: 'latitude' | 'longitude') => {
    ...
    if (minus_numbers_period_regex.test(value) && value !== '') {
      ...
      if (type === 'latitude') {
        if (!wait_list.includes(value)) set_coordinates((prev) => ({ ...prev, latitude: value === '' ? null: parseFloat(value)}));
        else {
        ...
        if (!wait_list.includes(value)) set_coordinates((prev) => ({ ...prev, longitude: value === '' ? null: parseFloat(value)}));
